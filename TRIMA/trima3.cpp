...
{//marquem com a usat el numero escollit i totes les restes que usen aquest valor
   // int i;
    int j;
   // for(i=0; i <= n; i++) mrestes[i][valor]*=(-1);
    //for(j=0; j <= n; j++) mrestes[valor][j]*=(-1);  //sha de veure
    mrestes[0][valor]*=(-1);
}
